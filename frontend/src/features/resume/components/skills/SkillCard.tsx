...
import { useDeleteSkill } from '../../hooks/resume.queries';
...
export const SkillCard: React.FC<SkillCardProps> = ({ skill, resumeId }) => {
  const [isExpanded, setIsExpanded] = useState(false);
  const [showDelete, setShowDelete] = useState(false);
  const { mutate: deleteSkill } = useDeleteSkill();

  const handleDelete = () => {
    deleteSkill({ id: resumeId, skillId: skill.id });
    setShowDelete(false);
  };
        ...
        >
          ...
          {isExpanded && (
            <div className="p-5 border-t border-slate-100 bg-slate-50/50">
              <SkillForm skill={skill} resumeId={resumeId} />
            </div>
          )}
