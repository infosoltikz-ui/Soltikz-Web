...
import { useDeleteEducation } from '../../hooks/resume.queries';
...
export const EducationCard: React.FC<EducationCardProps> = ({ education, resumeId }) => {
  const [isExpanded, setIsExpanded] = useState(false);
  const [showDelete, setShowDelete] = useState(false);
  const { mutate: deleteEducation } = useDeleteEducation();

  const handleDelete = () => {
    deleteEducation({ id: resumeId, eduId: education.id });
    setShowDelete(false);
  };
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-slate-500 text-sm truncate">
                {education.degree || ''} {education.startDate ? `• ${education.startDate} - ${education.currentlyStudying ? 'Present' : education.endDate || ''}` : ''}
              </p>
            </div>
