...
import { useDeleteAward } from '../../hooks/resume.queries';
...
export const AwardCard: React.FC<AwardCardProps> = ({ award, resumeId }) => {
  const [isExpanded, setIsExpanded] = useState(false);
  const [showDelete, setShowDelete] = useState(false);
  const { mutate: deleteAward } = useDeleteAward();

  const handleDelete = () => {
    deleteAward({ id: resumeId, awardId: award.id });
    setShowDelete(false);
  };

  return (
    <SortableItem id={award.id}>
        ...
        <div ref={setNodeRef} style={style} className={`bg-white border rounded-xl overflow-hidden transition-all duration-200 ${isDragging ? 'border-primary-400 shadow-lg scale-[1.02] z-10' : 'border-slate-200 hover:border-slate-300'}`}>
          <div className="flex items-center gap-3 p-4 bg-white cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
            ...
            <div className="flex-1 min-w-0">
              <h4 className="text-slate-900 font-medium truncate">{award.awardName || '(Not specified)'}</h4>
              <p className="text-slate-500 text-sm truncate">{award.issuer}{award.awardDate ? ` • ${award.awardDate}` : ''}</p>
            </div>
          ...
          {isExpanded && <div className="p-5 border-t border-slate-100 bg-slate-50/50"><AwardForm award={award} resumeId={resumeId} /></div>}
          <DeleteDialog isOpen={showDelete} onClose={() => setShowDelete(false)} onConfirm={handleDelete} title="Delete Award" description="Are you sure you want to delete this award?" />
        </div>
      )}
    </SortableItem>
  );
