...
import { useExperienceHistory } from '../../../hooks/useExperienceRewriter';
import { Clock, MessageSquareText } from 'lucide-react';
import { useAIStore } from '../../../store/useAIStore';

export const ExperienceHistory: React.FC<{ resumeId: string }> = ({ resumeId }) => {
  const { data: history, isLoading } = useExperienceHistory(resumeId);
  const { setRewrittenExperience } = useAIStore();
  ...
  if (!history || history.length === 0) {
    return (
      <div className="p-6 text-center text-slate-500">
        ...
        <p className="text-xs">No rewrite history found for this session.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4">
      <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3 px-2">Recent Rewrites</h4>
      <div className="flex gap-4 overflow-x-auto pb-2 snap-x">
        {history.map((conv: any) => {
          ...
          return (
            <div 
              key={conv.id}
              onClick={() => setRewrittenExperience(lastMsg.content)}
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-1.5 text-indigo-600">
                  ...
                  <span className="text-xs font-semibold">AI Rewritten</span>
                </div>
