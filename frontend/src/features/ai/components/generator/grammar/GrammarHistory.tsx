...
import { useGrammarHistory } from '../../../hooks/useGrammarGenerator';
import { Clock, MessageSquareText } from 'lucide-react';
import { useAIStore } from '../../../store/useAIStore';

export const GrammarHistory: React.FC<{ resumeId: string }> = ({ resumeId }) => {
  const { data: history, isLoading } = useGrammarHistory(resumeId);
  const { setGeneratedGrammar } = useAIStore();
  ...
  if (!history || history.length === 0) {
    return (
      <div className="p-6 text-center text-slate-500">
        ...
        <p className="text-xs">No optimization history found for this session.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4">
      <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3 px-2">Recent Optimizations</h4>
      <div className="flex gap-4 overflow-x-auto pb-2 snap-x">
        {history.map((conv: any) => {
          ...
          return (
            <div 
              key={conv.id}
              onClick={() => setGeneratedGrammar(lastMsg.content)}
