...
import { useUpdateAchievement } from '../../../../resume/hooks/resume.queries';
import { Check, X, Copy } from 'lucide-react';
import { RichTextEditor } from '../../../../resume/components/RichTextEditor';

export const AchievementPreview: React.FC<{ resumeId: string }> = ({ resumeId }) => {
  const { generatedAchievement, setGeneratedAchievement, achievementLoading, setGeneratorOpen, generatorSelectedId } = useAIStore();
  ...
  const currentDescription = achievement?.description || '';

  const handleAccept = () => {
    if (!generatorSelectedId || !generatedAchievement) return;
    ...
    updateAchievement(
      { 
        ...
        data: { description: generatedAchievement } 
      },
      {
        onSuccess: () => {
          setIsUpdating(false);
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(generatedAchievement.replace(/<[^>]*>?/gm, ''));
  };
  ...
  if (achievementLoading) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 space-y-4">
        ...
        <p className="text-sm text-slate-500 font-medium animate-pulse">Quantifying your impact...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-slate-800">Generated Achievement</h3>
      ...
      <div className="flex flex-col md:flex-row gap-4 h-full min-h-[300px]">
        ...
        <div className="flex-[2] flex flex-col bg-indigo-50/30 rounded-xl border border-indigo-100 overflow-hidden relative shadow-sm">
          ...
          <div className="p-4 flex-1 overflow-y-auto">
            <RichTextEditor 
              value={generatedAchievement} 
              onChange={setGeneratedAchievement} 
              placeholder="Generated achievement will appear here..." 
            />
          </div>
        </div>
      </div>
