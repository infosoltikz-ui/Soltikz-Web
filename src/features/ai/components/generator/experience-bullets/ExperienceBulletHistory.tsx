...
import { useExperienceBulletHistory } from '../../../hooks/useExperienceBulletGenerator';
import { Clock, MessageSquareText } from 'lucide-react';
import { useAIStore } from '../../../store/useAIStore';

export const ExperienceBulletHistory: React.FC<{ resumeId: string }> = ({ resumeId }) => {
  const { data: history, isLoading } = useExperienceBulletHistory(resumeId);
  const { setGeneratedExperienceBullets } = useAIStore();
  ...
  if (!history || history.length === 0) {
    return (
      <div className="p-6 text-center text-slate-500">
        ...
        <p className="text-xs">No bullet generation history found for this session.</p>
      </div>
    );
  }
      ...
      <div className="flex gap-4 overflow-x-auto pb-2 snap-x">
        {history.map((conv: any) => {
          ...
          return (
            <div 
              key={conv.id}
              onClick={() => setGeneratedExperienceBullets(lastMsg.content)}
